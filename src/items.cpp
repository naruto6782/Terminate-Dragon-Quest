...
void init_items_effects() {
    ...
    medicine.set_effect(//血瓶
        [](Hero* h, Monster* m){ h->change_HP(10,1.0); std::cout << "你使用了血瓶，回复10生命值！" << std::endl; }
    );
    poison.set_effect(//毒瓶
        [](Hero* h, Monster* m){ m->getStatusEffect()->setPoisoned(); std::cout << "你使用了毒瓶，怪物中毒！" << std::endl; }
    );
    ironMedicine.set_effect(//钢铁合剂
        [](Hero* h, Monster* m){ h->change_Defense(10,1.0); std::cout << "你使用了钢铁合剂，增加10防御！" << std::endl; }
    );
    angryDrink.set_effect(//愤怒合剂
        [](Hero* h, Monster* m){ h->change_Attack(10,1.0); std::cout << "你使用了愤怒合剂，增加10攻击！" << std::endl; }
    );
    antidote.set_effect(//解毒草
        [](Hero* h, Monster* m){ h->getStatusEffect()->resetPoisoned(); std::cout << "你使用了解毒草，解除中毒状态！" << std::endl; }
    );
    antibiotic.set_effect(//惊惶木
        [](Hero* h, Monster* m){ h->getStatusEffect()->resetStunned(); std::cout << "你使用了惊惶木，解除眩晕状态！" << std::endl; }
    );
    stunned.set_effect(//夜阑谣
        [](Hero* h, Monster* m){ m->getStatusEffect()->setStunned(); std::cout << "你使用了夜阑谣，怪物眩晕！" << std::endl; }
    );
    panacea.set_effect(//万灵药
        [](Hero* h, Monster* m){ h->getStatusEffect()->resetAll(); std::cout << "你使用了万灵药，解除所有状态效果！" << std::endl; }
    );
